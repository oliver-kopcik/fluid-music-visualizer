...
export function createFeatureReader() {
  const bassFast = new Envelope(0.03, 0.12);
  const bassSlow = new OnePole(0.4);
  const midSm = new OnePole(0.12);
  const trebleFast = new Envelope(0.03, 0.1);
  const rmsSm = new OnePole(0.1);
  const rmsSlow = new OnePole(0.4);
  const centroidSm = new OnePole(0.08, 0.5);
  const sustainSm = new OnePole(0.2);
