...
import { DEFAULT_SYNC_OFFSET } from '../app/simDefaults.js';

export function createMapping({ timeline, rng }) {
  ...
  const field = createField(rng);
  const feel = createFeel();
  /**
   * The atmosphere owns colour identity unless the UI overrides it.
   *
   * Held as a name rather than a built palette so it can change mid-playback; the first
   * version resolved this once at construction and setPalette silently did nothing.
   */
  ...
  /** Lookahead, so the visuals lead the audio — see DEFAULT_SYNC_OFFSET. */
